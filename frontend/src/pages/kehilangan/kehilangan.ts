  ...
  constructor(public navCtrl: NavController, public navParams: NavParams, public http: Http,
  	...
  	this.http.post(linkgetposts, JSON.stringify({jenis: "kehilangan"})).subscribe(data => {
          ...
          for (var i=0;i<this.response.data.length;i++) {
          	  ...
          	  let posts_elem =
          	  {usernim: this.response.data[i].usernim, nama: this.response.data[i].nama,
          	  jenis: this.response.data[i].jenis, judul: this.response.data[i].judul,
          	  deskripsi: this.response.data[i].deskripsi,
